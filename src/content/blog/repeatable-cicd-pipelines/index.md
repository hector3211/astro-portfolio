...
But looking back, that post ended at the design phase. I had planned the CI/CD workflow. I had drawn the boundary between plan and apply roles. I knew what I wanted to build.
...
Before this, my AWS work lived in a single account. That works when you are learning. It does not work when you want to separate production from experimentation, or when you want to give someone access to development without giving them access to everything.

I moved toward a multi-account setup with at least two environments:

- A **development** account for experiments, pull request previews, and anything I was not ready to trust yet.
- A **production** account for the resources that mattered.

That separation changes how you think about risk. In a single account, every change is potentially a production change because everything shares the same space. With separate accounts, you can make a mess in development, learn from it, and clean it up without touching the thing users depend on.
...
That is where IAM Identity Center became the center of the setup.
...
The biggest lesson was that identity is not a one-time setup task. It is a habit. Every time I add a new account, a new tool, or a new automation, I have to decide how it authenticates and what it is allowed to touch. IAM Identity Center forced me to make those decisions explicit instead of hidden in a long-lived access key somewhere.

## Billing Alarms and Cost Guardrails
...
I set up billing alarms through AWS Budgets with a few thresholds:
...
The thresholds themselves matter less than the discipline. The alarm made me think about cost every time I created a resource. It turned spending from something I checked at the end of the month into something I designed around.

I also learned that billing alarms are not just about catching mistakes. They are about giving yourself permission to experiment. When you know you will be told if something gets expensive, you feel safer trying things. The guardrail becomes a license to learn.
...
I used GitHub Actions with OIDC to authenticate to AWS. That meant no long-lived access keys stored in GitHub secrets. The workflow assumes a role directly, scoped to the account and permissions it needs. It is cleaner, safer, and easier to audit.
...
That gives me a plan file I can review before anything changes. The apply workflow is separate, protected by an environment approval gate, and uses a different IAM role with stronger permissions.

The plan role and apply role are intentionally not the same. The plan role needs enough access to read the environment and produce a plan. The apply role needs enough access to make changes. Splitting them means a compromised plan job cannot automatically modify infrastructure, and a mistake in the plan workflow has a smaller blast radius.

I also added container image builds and pushes into the same flow. Terraform can build the environment, but the pipeline has to build and ship the app into that environment. Keeping those steps in one place made the whole release easier to reason about.
...
Another test was drift. AWS changes, providers update, and state drifts. The pipeline did not eliminate drift, but it made drift visible. When Terraform showed unexpected changes, I could pause, run a refresh-only plan, and decide whether to update the code or the state.
...
**Automation reveals where your process is unclear.** If you cannot write a step in a workflow, you do not fully understand it. Writing the pipeline forced me to clarify decisions I had been making instinctively.

**Trust comes from visibility, not speed.** A slow pipeline that shows its work is more valuable than a fast one that hides it. People trust systems they can inspect.

**CI/CD is a communication tool.** It tells the team how changes are supposed to flow. It documents the process better than a README because the process is the code.
...
Infrastructure engineering in 2026 is less about knowing every AWS service and more about building systems other people can operate. Platform engineering, AI-assisted coding, and policy-as-code all push in the same direction: the engineer who can create safe, repeatable workflows is more valuable than the engineer who can run commands quickly.
...
2. **Set billing alarms on day one.** Even small environments deserve cost guardrails.
...
5. **Spend more time on approvals and review boundaries than on the automation.** The automation is easy. The trust design is hard.
6. **The goal is not to remove humans.** It is to make human decisions clearer, slower when they need to be, and easier to undo.

---

This project started as a follow-up to my Terraform work. It became a lesson in how infrastructure engineering really grows. The code matters, the tools matter, but the biggest change was in how I think about ownership, safety, and building things that can outlast my own attention.
